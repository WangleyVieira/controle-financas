...
@section('content')
    ...
    <div class="card" style="background-color:white">
        <div class="card-header">
            <h4>Listagem de Tipo de Categorias</h4><hr>
        </div>
        ...
        <div class="card-body">
            <div class="col-md-12 text-left mt-0 pt-0 mb-4">
                <button type="button" class="btn btn-success" data-toggle="modal" data-target="#modalCadastrar">
                    <i class="fas fa-plus-square"></i>&nbsp Cadastrar Tipo de Categoria
                </button>
            </div>
            @if ($tipoCategorias->isEmpty())
                <div>
                    <h1 class="alert-info px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">Não há Tipo de Categorias Cadastrados</h1>
                </div>
            @else
                    ...
                    <table id="datatables-reponsive" class="table table-bordered" style="width: 100%;">
                        ...
                        <tbody>
                            @foreach ($tipoCategorias as $tipoCategoria)
                                <tr>
                                    ...
                                    <td>{{ $tipoCategoria->descricao }}</td>
                                     <td>
                                        ...
                                        <button type="button" class="btn btn-warning m-1" data-toggle="modal" data-target="#modalAlterar{{ $tipoCategoria->id }}"
                                            title="excluir"><i class="fas fa-pen"></i></button>
                                    </td>
                                </tr>

                                <div class="modal fade" id="modalAlterar{{ $tipoCategoria->id }}" tabindex="-1" role="dialog"
                                    aria-labelledby="modalLabelExcluir{{ $tipoCategoria->id }}" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <form method="POST" class="form_prevent_multiple_submits" action="{{ route('configuracao.tipo_categoria.update', $tipoCategoria->id) }}">
                                                @csrf
                                                @method('PUT')
                                                <div class="modal-header btn-warning">
                                                    <h5 class="modal-title text-center" id="modalLabelExcluir{{ $tipoCategoria->id }}">
                                ...
                                <div class="modal fade" id="modalExcluir{{ $tipoCategoria->id }}" tabindex="-1" role="dialog"
                                    aria-labelledby="modalLabelExcluir{{ $tipoCategoria->id }}" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
    ...
    <div class="modal fade" id="modalCadastrar" tabindex="-1" role="dialog" aria-labelledby="modalLabelCadastrar" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content ">
                <form method="POST" class="form_prevent_multiple_submits" action="{{ route('configuracao.tipo_categoria.store') }}">
                    @csrf
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="form-group col-md-12">
                                <label class="form-label">*Descrição</label>
                                <input class="form-control @error('descricao') is-invalid @enderror" type="text" name="descricao" id="descricao" placeholder="Informe a descrição" value="{{ old('descricao') }}">
                                @error('descricao')
